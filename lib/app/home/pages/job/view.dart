      ...
      children: [
        ...
        TableEx.actions(
          children: [
            ...
            ElevatedButton(
              style: ButtonStyle(
                minimumSize: MaterialStateProperty.all(const Size(100, 80)), // 设置最小尺寸
                fixedSize: MaterialStateProperty.all(const Size(100, 80)),  // 设置固定尺寸
              ...
              return SingleChildScrollView(
                ...
                child: TablePage(
                  ...
                  tableData: TableData(
                    ...
                    theme: TableTheme(
                      headerColor: Colors.grey[200] ?? Colors.white,               // Header background color
                      headerTextColor: Colors.grey[800] ?? Colors.white,           // Header text color
                      rowColor: Colors.white,                      // Even row background
                      alternateRowColor: Colors.blue[50] ?? Colors.white,          // Odd row background
                      textColor: Colors.grey[900] ?? Colors.white,                 // Cell text color
                      border: Border.all(color: Colors.grey[300] ?? Colors.white, width: 1), // Table border
                    ),
                  ),
                  logic: logic,
                ),
...
class _TablePageState extends State<TablePage> {
  ...
  void toggleSelectAll() {
    if (selectedRows.length == widget.tableData.rows.length) {
      ...
      setState(() {
        selectedRows = List.generate(widget.tableData.rows.length, (index) => index);
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    return widget.loading
      ...
      headingRowColor: WidgetStateProperty.all(Colors.indigo.shade100), // 改进表头背景色
    );
  }
}
