...
import 'package:admin_flutter/api/class_api.dart';
...
class ClassLogic extends GetxController {
  ...
  void add() {
    form.add(
        reset: true,
        submit: (data) => {
          ClassApi.classInsert(params: data).then((value) {
            "插入成功!".toHint();
            find(size, page);
            Get.back();
          })
  ...
  void modify(Map<String, dynamic> d, int index) {
    ...
    form.edit(
        submit: (data) => {
          ClassApi.classUpdate(params: data).then((value) {
            "更新成功!".toHint();
            list.removeAt(index);
  ...
  void delete(Map<String, dynamic> d, int index) {
    ClassApi.classDelete(params: {"id": d["id"]}).then((value) {
      list.removeAt(index);
    });
  }
  ...
  void search(String key) {
    ClassApi.classSearch(params: {"key": key}).then((value) {
      refresh();
    });
  }
  ...
  Future<void> exportCurrentPageToCSV() async {
    ...
    File('$directory/classs_current_page.csv').writeAsStringSync(csv);
    "导出当前页成功!".toHint();
  }
  ...
  Future<void> exportAllToCSV() async {
    ...
    while (true) {
      var response = await ClassApi.classList(params: {
        "size": pageSize,
        "page": currentPage,
      });
    ...
    String csv = const ListToCsvConverter().convert(rows);
    File('$directory/classs_all_pages.csv').writeAsStringSync(csv);
    "导出全部成功!".toHint();
  }
