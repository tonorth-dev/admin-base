...
class ClassesLogic extends GetxController {
...
// 如果需要区分新增和更新的变量，可以保留原有的 u 前缀变量
  ...
  void edit(BuildContext context, Map<String, dynamic> classes) {
    currentEditClasses.value = RxMap<String, dynamic>(classes);
    ...
    DynamicInputDialog.show(
      ...
      child: ClassesEditForm(
        ...
        initialPhone: classes["phone"],
        initialPassword: classes["password"],
        initialProvince: classes["province"],
        initialCity: classes["city"],
        initialLeader: classes["leader"],
        initialInstitutionId: classes["institution_id"],
        initialInstitutionName: classes["institution_name"],
        initialClassId: classes["class_id"],
        initialClassName: classes["class_name"],
        initialReferrer: classes["referrer"],
        initialJobCode: classes["job_code"],
        initialJobName: classes["job_name"],
        initialJobDesc: classes["job_desc"],
        initialMajorIds: classes["major_ids"],
        initialMajorNames: classes["major_names"],
        initialExpireTime: classes["expire_time"],
        initialStatus: classes["status"].toString(),
      ),
      onSubmit: (formData) {
        print('提交的数据: $formData');
      },
  ...
  void generateAndOpenLink(
    ...
    final url =
        Uri.parse('http://localhost:8888/static/h5/?classesId=${item['id']}');
